...
<header>
<h3>ここではバナー一覧の内容を編集することが出来ます。</h3>
</header>
<section>
...
<td>
<?php if($saveTmpImageResult['banner_image']['result_code'] == UPLOAD_SUCCESS): ?>
<?php echo $this->DisplayImage->displayControlThumbnail($data['Banner']['banner_image']['name'], 2, 80, 300, true); ?>
<br>
<span>画像アップロード成功！</span>
<?php elseif($saveTmpImageResult['banner_image']['result_code'] == UPLOAD_FAILUER): ?>
<span class="requierd">※画像アップロード失敗！</span>
<?php elseif($saveTmpImageResult['banner_image']['result_code'] == UPLOAD_NO_DATA): ?>
...
<th>公開日</th>
<td>
<?php echo h($this->Html->dateFormat($data['Banner']['published']['year'].'-'.$data['Banner']['published']['month'].'-'.$data['Banner']['published']['day']." 00:00:00")); ?>
</td>
</tr>
<tr>
...
<p>
<?php echo $this->Form->create('Banner', array('type' => 'file', 'action' => 'control_edit')); ?>
<?php echo $this->Formhidden->hiddenVars(); ?>
<?php echo $this->Form->submit('入力画面に戻る', array('div' => false, 'id' => 'indexButton')); ?>
<?php echo $this->Form->end(); ?>
&nbsp;
<?php echo $this->Form->create('Banner', array('type' => 'file', 'action' => 'control_edit_complete')); ?>
<?php echo $this->Formhidden->hiddenVars(); ?>
